...
| `tables/bea-scb-gdp-2024-t1.pdf` | English (Latin) | BEA Survey of Current Business, "GDP and the Economy: Third Estimates for the Fourth Quarter of 2024" (April 2025), Table 1 (Real Gross Domestic Product and Related Measures), page 3, single-page qpdf excerpt | US-Gov work, public domain (17 U.S.C. §105) | Clean text layer; banded/group-ruled layout, 3-tier header. Cell-grid ground truth in `.cellgrid.tsv` (36×11) — see "Cell-grid accuracy corpus" below |
...
| `tables/fbi-nics-by-state-2026.pdf` | English (Latin) | FBI NICS Firearm Background Checks — Month/Year by State (Year 2026, Jan 1–May 31; snapshot 2026-06-16) | US-Gov work, public domain (17 U.S.C. §105) | **Held-out FULLY-RULED state-labeled lattice** (held-out table-quality corpus). Cell-grid ground truth in `.cellgrid.tsv` (56×14): col0=State/Territory unique anchor, per-row checksum sum(months)=Grand Total; char-spaced lattice extraction (content-clean, verbatim-low) — see "Cell-grid accuracy corpus" below. FBI overwrites the URL monthly; committed copy is a frozen snapshot |
| `tables/hhs-aspe-vsl-2024.pdf` | English (Latin) | HHS ASPE "Standard Values for Regulatory Analysis, 2024" (Jan 2024 Data Point), Table 1 (VSL Estimates by Year), p.2, single-page qpdf excerpt | US-Gov work, public domain (17 U.S.C. §105) | **Held-out FULLY-RULED** quality fixture #2 (with FBI NICS) — the fully-ruled coverage gate flips hard on these two. Cell-grid ground truth in `.cellgrid.tsv` (12×4): col0=Year unique anchor; page yields a 1×4 title fragment + the 12×4 data table, largest-table selection scores the data table; clean extraction — see "Cell-grid accuracy corpus" below |
| `tables/irs-p17zhs-rate-sched-2025.pdf` | Chinese (Simplified) | IRS Publication 17 (Simplified Chinese) 2025, Schedule X (single filers) of the 2025 tax rate schedules, PDF p.103, single-page qpdf excerpt | US-Gov work, public domain (17 U.S.C. §105) | **Held-out FULLY-RULED CJK** quality fixture (gate-independent bonus). Cell-grid ground truth in `.cellgrid.tsv` (8×4): col0=$-bracket unique anchor; CID-font/CJK diagnostic — bracket columns extract clean, tax-formula columns mis-split. Class confirmed by rendered rule coverage (interior verticals + per-cell rects). Page carries Schedules X/Y-1/Y-2/Z; largest-table selection scores Schedule X — see "Cell-grid accuracy corpus" below |
| `tables/erp-2024-tb1-gdp-pctchg.pdf` | English (Latin) | Economic Report of the President 2024, Table B-1 (Percent changes in real GDP, 1973–2023), PDF p.413, single-page qpdf excerpt | US-Gov work, public domain (17 U.S.C. §105) | **Held-out RECT-bordered** quality fixture (gate HELD). Cell-grid ground truth in `.cellgrid.tsv` (minimal 11×2: Year anchor + GDP %-change, annual rows 1973–1982). The detector drops the open Year column and collapses the rows → ~0% substantive: the measured capability gap — see "Cell-grid accuracy corpus" below |
| `tables/erp-2024-tb2-gdp-contrib.pdf` | English (Latin) | Economic Report of the President 2024, Table B-2 (Contributions to percent change in real GDP, 1973–2023), PDF p.415, single-page qpdf excerpt | US-Gov work, public domain (17 U.S.C. §105) | **Held-out RECT-bordered** quality fixture (gate HELD); second ERP table, same gap (single publisher CEA — cross-publisher generalization NOT demonstrated). Cell-grid ground truth in `.cellgrid.tsv` (minimal 11×2) — see "Cell-grid accuracy corpus" below |
| `tables/nass-cropan-2024-planted-harvested.pdf` | English (Latin) | USDA NASS Crop Production 2024 Summary (January 2025), "Principal Crops Area Planted and Harvested — States and United States: 2022–2024", page 7, single-page qpdf excerpt | US-Gov work, public domain (17 U.S.C. §105) | **Cross-publisher (USDA, non-CEA/ERP) held-out RECT-bordered** quality fixture (gate HELD). Cell-grid ground truth in `.cellgrid.tsv` (53×7, 3-tier header): col0=State unique anchor (open dot-leader); vertical column rules + outer frame, zero interior row rules; clean text layer — see "Cell-grid accuracy corpus" below |
| `tables/tw-dgbas-happiness-2025.pdf` | Chinese (Traditional) | Taiwan DGBAS, 表2 原國民幸福指數指標統計表 (Your Better Life Index indicators, Table 2), as of 2025-09-30 | **TW OGDL v1** (政府資料開放授權條款 v1.0, CC-BY compatible) | **Cross-publisher non-US held-out per-cell-grid (group-ruled+banded)** quality fixture; stroke-free, clean column split. Cell-grid ground truth in `.cellgrid.tsv` (45×9) — see "Cell-grid accuracy corpus" below |
| `tables/de-destatis-insolvenzen-2023.pdf` | German (Latin) | Destatis (Statistisches Bundesamt) Fachserie 2 Reihe 4.1, Unternehmensinsolvenzen nach ausgewählten Wirtschaftsbereichen, Juni 2023, p.6 | **DL-DE-BY-2.0** (Datenlizenz Deutschland Namensnennung 2.0) | **Cross-publisher non-US held-out per-cell-grid (group-ruled+banded)** quality fixture; stroke-free, the previously-G1-fused case PR-1 repairs (7 numeric columns collapsed into one data cell). Cell-grid ground truth in `.cellgrid.tsv` (66×9) — see "Cell-grid accuracy corpus" below |
| `tables/jp-mhlw-wage-structure-2024.pdf` | Japanese | Japan MHLW 賃金構造基本統計調査 (Basic Survey on Wage Structure) 2024 (令和6年), 第2表 性、年齢階級別賃金, page 8 | **Japan Government Standard Terms of Use v2.0** (政府標準利用規約, CC-BY 4.0 compatible) | **Cross-publisher CJK (Japanese) held-out per-cell-grid (group-ruled+banded)** quality fixture; PR-2 G2 row-recovery proof — 12 age-class rows collapsed into one band pre-fix. Cell-grid ground truth in `.cellgrid.tsv` (14×10) — see "Cell-grid accuracy corpus" below |
| `tables/jo-dos-health-2023-t14-7.pdf` | English + Arabic (bilingual) | Jordan Department of Statistics, Statistical Yearbook of Jordan 2023, Table 14.7 (Laboratory Tests at the Ministry of Health, 2019–2023), p.6, single-page qpdf excerpt | **Jordanian Open Government Data License (2019)** (best-available citation per Jordan's national open-data policy; not confirmed document-specific) | **First held-out SINGLE-AXIS-RULED** quality fixture — the single-axis-ruled Item 1 topology fix's committed regression lock (full-height vertical rules, zero horizontal rules between data rows). Bilingual English/Arabic mirror-column table; col0=Test Type unique anchor. Cell-grid ground truth in `.cellgrid.tsv` (12×7); extraction exact (7/7 cols) post-fix, was 14 cols pre-fix — see "Cell-grid accuracy corpus" below |
...
- FBI NICS Firearm Background Checks by State (monthly-overwritten URL; repo carries the 2026-06-16 frozen snapshot `fbi-nics-by-state-2026.pdf`): `https://www.fbi.gov/file-repository/cjis/nics_firearm_checks_-_month_year_by_state.pdf`
- HHS ASPE Standard Values for Regulatory Analysis, 2024 (7-page PDF; Table 1 on p.2; repo carries a single-page excerpt): `https://aspe.hhs.gov/sites/default/files/documents/cd2a1348ea0777b1aa918089e4965b8c/standard-ria-values.pdf` (landing page `https://aspe.hhs.gov/reports/standard-ria-values`)
- IRS Publication 17 (Simplified Chinese) 2025 (116-page PDF; the 2025 tax rate schedules incl. Schedule X on PDF p.103; repo carries a single-page excerpt): `https://www.irs.gov/pub/irs-pdf/p17zhs.pdf`
- Economic Report of the President 2024 (487-page PDF; Table B-1 on PDF p.413, Table B-2 on PDF p.415; repo carries single-page excerpts): `https://www.govinfo.gov/content/pkg/ERP-2024/pdf/ERP-2024.pdf`
- USDA NASS Crop Production 2024 Summary (January 2025; "Principal Crops" table on PDF p.7; repo carries a single-page excerpt): `https://www.nass.usda.gov/Publications/Todays_Reports/reports/cropan25.pdf`
...
- BEA Survey of Current Business (April 2025 GDP article, 21-page PDF; Table 1 on p.3; repo carries a single-page excerpt): `https://apps.bea.gov/scb/issues/2025/04-april/pdf/0425-gdp-economy.pdf`
...
from GoPDF `Words()`/`Lines()`/`Blocks()` output. When no structured companion is published
or reachable (e.g. the ERP B-1/B-2 GDP columns, whose BEA source was unreachable at
authoring time), the rendered page of the authoritative published document is the source of
record and a GoPDF-independent extractor (poppler `pdftotext`) supplies the digit
cross-check — independence from GoPDF is preserved either way. NIST HB44 is deliberately
**excluded** from this set (it was the cell-seg spike's tuning source; scoring against it
would flatter the number) — it stays a bordered/lattice example only.
...
| `tables/bea-scb-gdp-2024-t1.cellgrid.tsv` | group-ruled+banded (banded) | 36×11 | 3 | none (blind-authored, 2-transcriber cross-check) | Cross-publisher (non-EIA) held-out banded target; alternating-fill row bands; currently row-over-merged by Tables() |
...
| `tables/fbi-nics-by-state-2026.cellgrid.tsv` | **fully-ruled (stroke lattice), state-labeled** | 56×14 | 1 | self-checksum: per row, sum(Jan–May months) = Grand Total (55/55 rows) | First **held-out fully-ruled** quality fixture for the held-out table-quality measurement corpus (`heldOut=true`, scored by `TestPublicTablesQualityCorpus`). 55 jurisdiction rows (states + DC + Guam + Mariana Islands + Puerto Rico + Virgin Islands), one per row; col0 = unique row anchor. June–December columns empty (no data in this 2026 snapshot). GoPDF emits char-spaced tokens (looseCell content match; verbatim low by design) — threshold-naïve (not EPA/IRS/NIST), so a real within-class generalization measurement |
| `tables/hhs-aspe-vsl-2024.cellgrid.tsv` | **fully-ruled** | 12×4 | 1 | ASPE Data Point body text ($6.1M/$13.1M/$19.9M for 2024) + rendered page | Second gate-bearing **held-out fully-ruled** quality fixture (with NICS); the fully-ruled coverage gate is now hard (`t.Errorf` if <2). col0=Year unique anchor. GoPDF returns a 1×4 title fragment + the 12×4 data table; the harness largest-table selection scores the data table. Clean extraction (substantive content+verbatim 100%) |
| `tables/irs-p17zhs-rate-sched-2025.cellgrid.tsv` | **fully-ruled (CJK / Simplified Chinese)** | 8×4 | 1 | IRS 2025 tax rate schedules (Rev. Proc. 2024-40) + rendered page | **Held-out fully-ruled CJK bonus** (gate-independent — the gate rests on NICS + ASPE). col0=$-bracket lower-bound unique anchor. Class confirmed by **rendered rule coverage** (content cell-dump: interior vertical rules at x=176/306/436 + per-cell rects), not detector self-attestation. CID-font diagnostic: bracket columns extract clean, the tax-formula columns (税额为 / 超过部分) mis-split → substantive ~29%. Page carries Schedules X/Y-1/Y-2/Z; largest-table selection scores Schedule X (single filers) |
| `tables/erp-2024-tb1-gdp-pctchg.cellgrid.tsv` | **rect-bordered** (gate HELD) | 11×2 (minimal) | 1 | rendered page + pdftotext (poppler; BEA NIPA fetch unavailable) | **Held-out rect-bordered** — minimal golden (col0=Year anchor + col1=GDP %-change, annual rows 1973–1982) that PROVES the gap: the detector drops the open Year column and collapses the rows → **~0% substantive**. The full ~47×13 grid + the detector fix (open-anchor-column recovery + row inference) are scoped in a committed follow-up plan; the rect-bordered coverage gate is HELD (loud diagnostic, not green) until it ships. Negative sign printed as en-dash |
| `tables/erp-2024-tb2-gdp-contrib.cellgrid.tsv` | **rect-bordered** (gate HELD) | 11×2 (minimal) | 1 | rendered page + pdftotext (poppler) | Second ERP rect-bordered held-out fixture, same ~0% gap. Both fixtures are ERP/CEA (single publisher) — ≥2 coverage met, **cross-publisher generalization NOT demonstrated** (reopen trigger: a structurally-distinct, different-publisher rect-bordered federal-PD table) |
| `tables/nass-cropan-2024-planted-harvested.cellgrid.tsv` | **rect-bordered** (gate HELD) | 53×7 | 3 | none (blind-authored, full text-layer value cross-check 300/300) | **Cross-publisher (USDA, non-CEA/ERP) held-out rect-bordered generalization fixture**; open dot-leader State anchor; clean font. Third gate-bearing rect-bordered held-out quality fixture |
| `tables/tw-dgbas-happiness-2025.cellgrid.tsv` | **group-ruled+banded (per-cell-grid, CJK Traditional Chinese)** | 45×9 | 1 | none (blind-authored, 2-transcriber reconciled; numeric cells 100% A/B agreement, image-verified) | **Cross-publisher (Taiwan DGBAS, non-US) PR-1 de-bias fixture** — the CLEAN per-cell-grid case. col1=指標 unique anchor (col0 領域 = section label, blank on continuation). Exercises PR-1 `inferColumnCuts` v-edge column recovery end-to-end. 3 anchor-column footnote-mark cells `# known-ceiling` (⑩/⑪/⑫ glyphs + 受雇/受僱 ambiguous). Substantive ~55% — footnote-circle glyphs decode shifted vs the printed mark, so the few circle-bearing rows mis-anchor (an anchor-matching limit, not a reconstruction failure) |
| `tables/de-destatis-insolvenzen-2023.cellgrid.tsv` | **group-ruled+banded (per-cell-grid, German)** | 66×9 | 1 | none (blind-authored, 2-transcriber reconciled + arithmetic cross-check col4=col2+col3 & parent sums; ~94% raw A/B agreement) | **Cross-publisher (Germany Destatis, non-US) PR-1 de-bias fixture** — the previously-G1-FUSED case PR-1 repairs (the 7 numeric columns collapsed into one data cell before `inferColumnCuts`; **~97% substantive after**). col0=Nr. der Klassifikation unique WZ-code anchor. 13 genuinely-illegible numeric cells `# known-ceiling`. Dense German numerics are transcription-hard → a best-effort honest golden, not pixel-perfect |
| `tables/jp-mhlw-wage-structure-2024.cellgrid.tsv` | **group-ruled+banded (per-cell-grid, CJK Japanese)** | 14×10 | 2 | none (blind-authored, 2-transcriber reconciled; 108/108 numeric cells 100% A/B agreement + every 賃金格差 cell arithmetic-verified by 賃金/(20～24 賃金)×100; 0 known-ceiling) | **Cross-publisher (Japan MHLW) held-out per-cell-grid (group-ruled+banded) PR-2 de-bias fixture** — the G2 vertical row-collapse PR-2 repairs (12 age-class rows stacked into one band pre-fix). col0=年齢階級 unique age-class anchor. Out-of-sample row-recovery proof; the first Japanese-language held-out fixture |
| `tables/jo-dos-health-2023-t14-7.cellgrid.tsv` | **single-axis-ruled (bilingual English/Arabic)** | 12×7 | 1 | direct PDF Read (render truth), digit-by-digit cross-check against the page image | **First held-out single-axis-ruled fixture** — the single-axis-ruled Item 1 topology fix's committed regression lock. col0=Test Type unique anchor (includes the Total and Source footer rows). Extraction exact (7/7 cols, byte-identical to render truth) post-fix; single-axis-ruled is not yet in `inScopeQualityClasses`, so this fixture is scored as a diagnostic only, not gate-bearing |

Several real PDFs had their document metadata stripped (`qpdf --remove-info --remove-metadata`)
...
dropped; the `tw-dgbas-happiness-2025.pdf` file likewise carried a populated `/Info /Author`,
and `de-destatis-insolvenzen-2023.pdf` was stripped for parity. The page content and text layer
are unmodified, so the `.golden.txt` snippets and `.cellgrid.tsv` grids are unaffected (verified:
the corpus golden + quality tests pass identically post-strip). This matters because the PII scan
skips `testdata/`, so a personal name here would otherwise bypass automated checks.
