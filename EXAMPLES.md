...
- **Subsetted Walkman-Chanakya 905** (fonts whose content codes are remapped to a compact subset range,
  ...
  (~42,862 words across a 233-page Indian statistical appendix); `Reader.Warnings()` stays silent for a
  cleanly-recovered font. **Known limitation:** precomposed-ligature glyphs (~0.1% of recovered words)
...
- **Subsetted Walkman-Chanakya 905**: mostly recovered — ~99.9% of recovered words correct on the
  one tested fixture; warning stays silent for a cleanly-recovered font. **Known limitation:**
  precomposed-ligature glyphs (~0.1% of recovered words) may mis-decode; the orthographically-invalid
