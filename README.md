...
- `Reader.Warnings()` returns deterministic diagnostics for silently degraded extraction, including missing or broken `/ToUnicode`, fallback CJK encodings, unknown encodings, unmappable glyphs, legacy non-Unicode Indic fonts (Kruti Dev / DevLys / Walkman-Chanakya — canonical simple Kruti Dev 010 and subsetted Walkman-Chanakya 905 are transcoded to real Unicode Devanagari; the warning is recovery-aware and stays silent for cleanly-recovered fonts; Walkman-Chanakya 905 has a known precomposed-ligature limitation (~0.1% of recovered words) detected at the Tables() level — see EXAMPLES.md; composite/Type0 Kruti and DevLys still decode to Latin gibberish), and unsupported stream filters.
