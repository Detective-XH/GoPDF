...
- **Legacy non-Unicode Indic fonts** — government statistical PDFs that render Devanagari through a legacy Latin-mapped font (Kruti Dev / DevLys / Walkman-Chanakya) — are detected, and **simple Kruti Dev 010** is transcoded to real, searchable Unicode on every reading surface (`Words`/`Lines`/`Blocks`/`Tables`/`GetPlainText`), recovering text that was previously extractable only as Latin gibberish. Variants not yet transcoded (composite/Type0 Kruti, subsetted Walkman, DevLys) are flagged rather than silently returned. See [EXAMPLES.md](EXAMPLES.md).
