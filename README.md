...
- `Page.Tables()` reconstructs **ruled (lattice) tables** — those drawn with visible cell borders — into a grid of cell strings (`Table.Cells[row][col]`), reading the ruling lines from `Content.Stroke` and thin `Content.Rect`. Stable, with reconstruction accuracy locked by corpus regression gates on the documented scope (fully-ruled lattices plus structurally-recovered half-open edge columns). Borderless and partially-ruled/banded tables are out of scope — best-effort, not a contract: they yield no table or an incomplete grid. See [EXAMPLES.md](EXAMPLES.md).
