...
- `Reader.Warnings()` returns deterministic diagnostics for silently degraded extraction, including missing or broken `/ToUnicode`, fallback CJK encodings, unknown encodings, unmappable glyphs, legacy non-Unicode Indic fonts (Kruti Dev / DevLys / Walkman-Chanakya — their text decodes to Latin gibberish on every surface), and unsupported stream filters.
