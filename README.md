...
| Ruled-table reconstruction — fully-ruled + rect-bordered + per-cell-grid (experimental) | `Page.Tables` |
...
- Cross-page **diagonal watermarks** (the same skew-rotated stamp printed on ~every page) are detected and dropped from the reading-order surfaces (`Words`/`Lines`/`Blocks`/`Tables`) so a watermark glyph cannot fuse into a value (e.g. `11144` read as `111Ê44`); page-specific rotated content such as chart-axis labels is preserved, and `Page.Content()`/`Page.Texts()` keep every glyph unfiltered.
...
- `Page.Tables()` (experimental) reconstructs **ruled tables** — those drawn with visible cell borders — into a grid of cell strings (`Table.Cells[row][col]`), reading the ruling lines from `Content.Stroke` and thin `Content.Rect`. Two table classes are locked by corpus regression gates: **fully-ruled lattices** and **rect-bordered tables** (interior cells closed by rules within an outer frame, ~98% cross-publisher); plus structurally-recovered half-open edge columns. **Per-cell-grid tables** (stroke-free, common in non-US/multilingual statistical PDFs) now reconstruct with column- and row-recovery, but this class is still stabilizing — best-effort, not yet a locked contract. **Proportional Type0 / Identity-H text** (Latin/Cyrillic wrapped in a CJK-subset font) now extracts with correct per-CID widths, fixing label↔value character-interleaving. Borderless and partially-ruled/banded tables remain out of documented scope — best-effort, not a contract: they may yield no table or an incomplete grid. Each detected table also carries a **detection-relative `Confidence` and per-table `Warnings`** (for example a phantom-table flag, or a `legacy_font_text` flag when a table's labels were rendered through a legacy non-Unicode Indic font — canonical **simple Kruti Dev 010** and **subsetted Walkman-Chanakya 905** are now transcoded to real, searchable Unicode (Walkman-Chanakya 905 carries a known precomposed-ligature limitation — ~0.1% of recovered words may mis-decode; the orthographically-invalid subset is flagged via `legacy_font_text` at `Confidence Low` with `misdecoded_clusters=N` in the detail), composite (Type0) **Kruti Dev 010** on both a CFF- and a TrueType-descendant fixture is also now transcoded via a `/ToUnicode` CID→keystroke bridge, while still-undecoded variants (other composite Kruti families, DevLys) keep the flag and stay gibberish rather than being silently returned as if trustworthy), and `Page.TableRegions()` returns page-space bounding boxes 1:1 with `Tables()` — honest per-table quality signals and locations a RAG consumer can use to flag or fall back on a low-confidence grid. See [EXAMPLES.md](EXAMPLES.md).
...
- **Legacy non-Unicode Indic fonts** — government statistical PDFs that render Devanagari through a legacy Latin-mapped font (Kruti Dev / DevLys / Walkman-Chanakya) — are detected, and several families/variants are now transcoded to real, searchable Unicode Devanagari on every reading surface (`Words`/`Lines`/`Blocks`/`Tables`/`GetPlainText`): **canonical-coded simple Kruti Dev 010** (fully recovered), **subsetted Walkman-Chanakya 905** (mostly recovered — ~99.9% of recovered words correct on the one tested statistical-appendix fixture; known limitation: precomposed-ligature glyphs ~0.1% of words may mis-decode, with the orthographically-invalid subset flagged via `legacy_font_text` at `Confidence Low`), and **composite (Type0/CID-keyed) Kruti Dev 010** (recovered on both a CFF- and a TrueType-descendant fixture via a `/ToUnicode` CID→keystroke bridge — the same recovery mechanism, reused unchanged, just sourced from `/ToUnicode` instead of `/Encoding /Differences`). Still-undecoded variants (other composite legacy Indic fonts, DevLys) are flagged rather than silently returned. See [EXAMPLES.md](EXAMPLES.md).
...
- `Reader.Warnings()` returns deterministic diagnostics for silently degraded extraction, including missing or broken `/ToUnicode`, fallback CJK encodings, unknown encodings, unmappable glyphs, legacy non-Unicode Indic fonts (Kruti Dev / DevLys / Walkman-Chanakya — canonical simple Kruti Dev 010, subsetted Walkman-Chanakya 905, and composite (Type0) Kruti Dev 010 are transcoded to real Unicode Devanagari; the warning is recovery-aware and stays silent for cleanly-recovered fonts; Walkman-Chanakya 905 has a known precomposed-ligature limitation (~0.1% of recovered words) detected at the Tables() level — see EXAMPLES.md; other composite Kruti families and DevLys still decode to Latin gibberish), and unsupported stream filters.
...
- Table reconstruction (`Page.Tables()`, experimental): two table classes are locked by corpus regression gates — **fully-ruled lattices** and **rect-bordered tables** (interior cells closed by rules within an outer frame, ~98% cross-publisher) — plus structurally-recovered half-open edge columns. **Per-cell-grid tables** (stroke-free, common in non-US/multilingual statistical PDFs) now reconstruct with column- and row-recovery, but remain best-effort, not yet a locked contract. **Proportional Type0 / Identity-H text** (Latin/Cyrillic wrapped in a CJK-subset font) now extracts with correct per-CID widths, fixing label↔value character-interleaving. Borderless and partially-ruled/banded tables remain out of documented scope (best-effort, not a contract — they may yield no table or an incomplete/merged grid).
...
maintainer's hardware-backed SSH keys; v0.8.7 was signed with:
...
git -c gpg.ssh.allowedSignersFile=./allowed_signers tag -v v0.8.7
